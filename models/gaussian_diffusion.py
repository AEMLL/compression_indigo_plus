...
from .losses import normal_kl, discretized_gaussian_log_likelihood,iqa_loss
...
class GaussianDiffusion:
    ...
    def ddim_sample_loop_progressive(
        ...
        if self.configs_indigo.finetune_lr:
            optimizer = th.optim.Adam(self.model_inn.parameters(), lr=self.configs_indigo.finetune_lr)
        ...
        for i in indices:
            ...
            if self.configs_indigo.finetune_lr and i>self.configs_indigo.finetune_end_step:

                self.model_inn.train()
                for param in self.model_inn.parameters():
                    param.requires_grad = True

                inninput = (out["pred_xstart"] / 2 + 0.5).detach_()

                cn_finetune, dn_finetune = self.model_inn.dae_f(x=inninput, cond=cond)

                mse_loss = th.nn.MSELoss().cuda()
                loss = mse_loss(y0_norm, cn_finetune)
