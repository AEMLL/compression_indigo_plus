...
from utils import util_wavelet
import copy
import torch
...
class BaseSampler:
    def __init__(self, configs, configs_indigo=None, im_size=512, use_fp16=False):
        '''
        Input:
            configs: config, see the yaml file in folder ./configs/sample/
        '''
        self.configs = configs
        self.configs_indigo = configs_indigo
    ...
    def load_model(self, model, ckpt_path=None,strict=False):
        # if self.rank == 0:
...
class DifFaceSampler(BaseSampler):
    ...
    def build_model(self):
        ...
        from inn.network_kdsr import KD_IDE
        net_kdsr = KD_IDE().cuda()
        state_dict= torch.load("weights/KDSR/KDSRS-GANV2.pth")["params_ema"]
        filtered_state_dict = {k.replace("E_st.", ""): v for k, v in state_dict.items() if "E_st." in k}
        net_kdsr.load_state_dict(filtered_state_dict)
        self.net_kdsr = net_kdsr
        self.net_kdsr.eval()
        self.freeze_model(self.net_kdsr)

        from inn.network_cinn import RWNN
        basic_block_type = "rb_cres_kdsr"
        netG = RWNN(offset=0.5, LiftNet_iter=2, basic_block_type=basic_block_type, net_kdsr=self.net_kdsr, kdsr=1).cuda()

        self.model_inn = netG
        self.load_model(netG, self.configs_indigo.INN_dir,strict=False)
        self.initial_inn_state = copy.deepcopy(self.model_inn.state_dict())
        self.model_inn.eval()
        self.freeze_model(self.model_inn)
    ...
    def sample_func_ir_aligned(
        ...
        if self.configs_indigo.finetune_lr:
            self.model_inn.load_state_dict(self.initial_inn_state)
            self.model_inn.eval()
            self.freeze_model(self.model_inn)

        start_time = time.time()
        if need_restoration:
            im_hq = self.restore_func(y0_resize, model_kwargs_ir)
        else:
            im_hq = y0_resize
        ...
        sample = self.diffusion.ddim_sample_loop(
                ...
                configs_indigo=self.configs_indigo,
                model_inn=self.model_inn,cond=cond
                )
        sample = util_image.normalize_th(sample, reverse=True).clamp(0.0, 1.0)
        sample = util_wavelet.wavelet_reconstruction(sample, im_hq).clamp(0.0, 1.0)
        end_time = time.time()
...
# Unconditioned INDIGO, specific for compression
class CompressionSampler(BaseSampler):
    def restore_func(self, y0, model_kwargs_ir):
        if model_kwargs_ir is None:
            inputs = y0
        elif 'mask' in model_kwargs_ir:
